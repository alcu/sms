function [xout yout] = pick_point_counterclock(x, y)
% function [xout yout] = pick_point_counterclock(x, y)
%
% Picks the "most counterclockwise" point out of 2 points. If only 1 point is
% inputted, the same point is outputted.
%
% Inputs:
...
%{
if ang2 > pi/2 & ang1 < -pi/2
    xout = x(1); yout = y(1);
elseif ang1 > pi/2 & ang2 < -pi/2
    xout = x(2); yout = y(2);
else % Points are not around boundary of +/-pi: Pick point with largest angle.
    if ang1 < ang2
        xout = x(2); yout = y(2);
    else
        xout = x(1); yout = y(1);
...
tmp = unwrap([ang1 ang2]);
ang1 = tmp(1);
ang2 = tmp(2);
% Pick point with largest angle.
if ang1 < ang2
    xout = x(2); yout = y(2);
else
    xout = x(1); yout = y(1);
